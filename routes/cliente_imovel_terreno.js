...
function render_index(req, res) {
    res.render('cliente_imovel_terreno', {empresa: settings.empresa});
};
...
exports.gravar = function (req, res) {
    ...
    connection.connect();
    if (data.codigo==0) {
        connection.query('insert into tb_cliente_imovel_terreno (tipo, nome, cep, estado, cidade,'+
        ...
        connection.query('update tb_cliente_imovel_terreno set tipo=?, nome=?, cep=?, estado=?, cidade=?,'+
...
exports.codigo = function (req, res) {
    ...
    ' b.nome as bairro_, e.nome as endereco_ from tb_cliente_imovel_terreno p'+
...
exports.cliente_imovel_terreno_todas = function (req, res) {
    var connection = mysql.createConnection(settings.dbConect);

    connection.connect();
    connection.query('SELECT * from tb_cliente_imovel_terreno order by nome', function(err, rows, fields) {
        if (!err)
            res.json({cliente_imovel_terreno_todas: rows})
        else
            console.log('Error while performing Query.')
    });
    connection.end();
...
exports.cliente_imovel_terreno_nome = function (req, res) {
    var connection = mysql.createConnection(settings.dbConect);
    var txt = req.query.txt;

    connection.connect();
    connection.query("select codigo, nome from tb_cliente_imovel_terreno"+
...
exports.localizar = function (req, res) {
    ...
    sql += " FROM tb_cliente_imovel_terreno p left join tb_cliente_imovel_terreno_fisica f on f.pessoa=p.codigo";
    sql += " left join tb_cliente_imovel_terreno_juridica j on j.pessoa=p.codigo Where";
...
exports.dlg_localizar = function (req, res) {
    res.render('cliente_imovel_terreno_dlg_localizar');
};

exports.dlg_apagar = function (req, res) {
    res.render('cliente_imovel_terreno_dlg_apagar');
};